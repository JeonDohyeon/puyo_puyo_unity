...
public class Puyo : MonoBehaviour
{
    ...
    public void RotateLeft(){
        ...
        if(ValidRotate(vect)){
            ...
            if(Vector3.Distance(puyoUnitPos + down, transform.position) == 0) {
                if(ValidMove(left)) {
                    Move(left, transform);
                    Move(vect, unitArray[1].transform);
                    puyoSpinCount = 0;
            ...
            } else if(Vector3.Distance(puyoUnitPos + left, transform.position) == 0 || Vector3.Distance(puyoUnitPos + right, transform.position) == 0) {
                if(ValidMove(up)) {
                    Move(up, transform);
                    Move(vect, unitArray[1].transform);
                    puyoSpinCount = 0;
            ...
            if(puyoSpinCount == 2){
                Vector3 vect180 = GetOppositeDegRotationVector();
                Move(vect180, unitArray[1].transform);
                puyoSpinCount = 0;
            }
        }
    }
    ...
    Vector3 GetOppositeDegRotationVector(){
        Vector3 puyoUnitPos = RoundVector(unitArray[1].transform.position);
